...
class RekamMedisController extends Controller
{
    ...
    public function actionIndextfidf()
    {   
        // Set the endpoint URL
        $endpoint = 'http://localhost:105/api/resource';
        // print_r(Yii::$app->request->queryParams);
        $search_query = Yii::$app->request->post('Search');
        ...
        // Make the POST request using file_get_contents
        $response = file_get_contents($endpoint, false, $context);

        // Check for errors
        $pesan_error = "";
        $my_query;
        $allowedDocumentIds = array();
        if ($response === FALSE) {
            $pesan_error = 'Error making the request';
        } else {
            // Print the response
            // echo $response;
            $data = json_decode($response, true);
            $my_query = $data['name'];
            $term_list = $data['token'];
            $allowedDocumentIds = $data['value'];
            //print_r($allowedDocumentIds);die();
        }
        // die();

        $cariRM = isset($_POST['rm'])? $_POST['rm']:"";
        // $allowedDocumentIds = [252,144, 77, 19, 18, 15, 7, 6, 5, 4, 3, 2, 1];
        if($cariRM!="")
            \Yii::$app->getSession()->setFlash('success', 'Pencarian Rekam Medis dengan kata kunci : "'.HTML::encode($cariRM).'"');

        $searchModel = new RekamMedisSearch();
        $dataProvider = $searchModel->searchtfidf(Yii::$app->request->queryParams,$cariRM,$allowedDocumentIds);

        return $this->render('indextfidf', [
            'search_query' => $my_query,
            'term' => $term_list,
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    ...
    {   
        $id = Yii::$app->security->decryptByKey( utf8_decode(Yii::$app->request->get('id')), Yii::$app->params['kunciInggris'] );
        return True;//$this->findModel($id)->user_id == Yii::$app->user->identity->id;
    }
